...
| `INFINITEMODEL_CUDA_GRAPH=<ctx>` | Opt-in CUDA-graph decode (single-node, standard-attention, uniform-CUDA models; `<ctx>` sizes the StaticCache mirror — set it to the serving ctx). Copy-handoff design: prefill/verify stay on the proven eager DynamicCache path; the first decode captures `model.forward` over a StaticCache mirror, the second **replays at a new position** and self-checks against the eager DynamicCache decode — activates only on a match (`[cudagraph] decode ACTIVE`), else latches off permanently and serving stays eager (byte-identical). Default-OFF (inert without this var). **HIP/ROCm gfx1151: tested 2026-07-08 (TheRock torch 2.12.0a0+rocm7.13, HIP 7.13.60980, llama-3.3-70b int4) — capture and replay execute without error, but the replayed logits are ~71–74% off (`rel=0.740`/`0.714`, two independent loads); the self-check DISABLES it every time. NVIDIA is the intended target. Since `db47ae1` you cannot re-enable it on ROCm by setting this var at all: `_graph_enabled` carries an explicit `not torch.version.hip` term (`shard_forward.py:985`), added because ROCm reports `device.type == "cuda"` so the gate's device check did *not* exclude HIP, and a HIP graph can emit plausible-but-wrong int4 logits the self-check need not catch. The var is therefore *inert* on ROCm; a retest on a newer TheRock/HIP has to lift that guard *as well as* set the var.** |
...
| + **measured expert-row de-alias** (#4c: gate_up PAD 1.7×) | **16.3** | **7.8×** |
...
rel=0.714 -> DISABLED (eager)`) and serving stays eager and coherent. A retest on a newer
TheRock/HIP is worth doing, but **setting the env var alone will no longer reach the code**:
`db47ae1` hard-gated `_graph_enabled` with `not torch.version.hip` (`shard_forward.py:985`) so the
flag targets real CUDA only, which makes `INFINITEMODEL_CUDA_GRAPH` inert on every ROCm box. Budget
the retest as "lift the guard in `shard_forward.py`, then measure" — otherwise you will spend a
session watching a flag do nothing and conclude, wrongly, that the model was never eligible.
...
1. **CUDA/HIP-graph capture of the decode step — the big one (measured; opt-in landed).** Now unblocked: fusing the
   ...
   target. That outcome was then made structural: `db47ae1` added `not torch.version.hip` to the
   `_graph_enabled` gate (`shard_forward.py:985`), so on ROCm the env var is **inert** rather than
   merely self-disabled — no capture is attempted. Anyone re-running this experiment must remove
   that guard first; the env var by itself will produce no `[cudagraph]` log line at all. Note the
   eager overhead this lever buys back is also smaller than first simmed: a clean
