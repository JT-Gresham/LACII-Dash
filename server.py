...
VERSION = "0.3.38.4"  # version tag only; full changelog -> CHANGELOG.md
...
MODELS: dict[str, tuple[str, str]] = {
    ...
    # NOTE (#spec, measured 2026-07-27): qwen3-30b-a3b-instruct was briefly paired with a
    # Qwen3-0.6B draft to try speculative decode, and it MEASURED A LOSS on gfx1151 — 10.2 vs
    # 27.2 tok/s plain (2.7x SLOWER), 34% draft acceptance. A 3.3B-ACTIVE MoE's verify sweep is
    # already cheap, so the K draft steps + verify (which serialize on the one gfx1151 GPU) cost
    # far more than the ~2.35 tok/round they save. The pairing also forced a draft download+attach
    # on EVERY load (the _load_draft call fires whenever draft_id != target_id, not just draft_gpu),
    # thrashing unified-memory SVM. So it's intentionally left as draft==target (spec OFF). Spec
    # stays reserved for big DENSE targets (llama-3.3-70b) where the verify sweep dominates.
...
from formats import (_iso, _digest, _human_params, _details, _model_info,
                     _to_id_list, _safe_decode, _decode_visible, IncrementalDetok,
