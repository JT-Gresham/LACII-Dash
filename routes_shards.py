"""routes_shards: the shard-cache / distributed-packing / weight-serving routes, relocated
VERBATIM from routes_lifecycle.py (code-split Inc 6): /shard_status, /verify_shards,
/pack_result, /pack_probe, /compile_dist, /compile_shards, plus the worker-facing DATA-PLANE
routes /weights, /weights_tp, /experts (workers stream model weights through these during a
load -- treat this file as data-plane critical), plus the PARKED debug routes /mtp_probe +
/modelcode (#91 MTP self-spec is closed/gated off; kept for future forensics, dead traffic).

Bodies BYTE-IDENTICAL to the originals; module globals (engine, registry, MODELS, JSONResponse,
Request, resolve_model_name, _controller_model_dir, ...) are injected at startup by state.bind()
-- see state.py. build_app() calls register(app). Controller-only leaf; in EXTRA_UPDATE_FILES.
"""
